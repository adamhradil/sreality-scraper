...
class SrealitySpider(scrapy.Spider):
    ...
    def __init__(self, spider_settings: dict, *args, **kwargs):
        ...
        self.params = [
            ...
            (
                "region_entity_id",
                params_mapping[spider_settings["location"]]["region_entity_id"],
            ),
            (
                "region_entity_type",
                params_mapping[spider_settings["location"]]["region_entity_type"],
            ),
            ("per_page", "60"),
        ]
        self.base_url = "https://www.sreality.cz/api/cs/v2/estates?" + urlencode(
            self.params
        )
    ...
    def parse_page(self, data):
        if data["_embedded"]["estates"]:
            estates = [
                estate["_links"]["self"]["href"]
                for estate in data["_embedded"]["estates"]
                if estate["region_tip"] == 0
            ]
            for href in estates:
                yield scrapy.Request(
                    f"https://www.sreality.cz/api{href}", callback=self.parse_estate
                )
    ...
    def parse_estate(self, response):
        ...
        sreality_item["service_fees"] = self.get_first_or_none(
            data.get("items"), "Poznámka k ceně"
        )
        # security_deposit =
        ...
        sreality_item["disposition"] = (
            data.get("seo").get("category_sub_cb")
            if data.get("seo").get("category_main_cb") == 1
            else self.convert_room_count(
                data.get("recommendations_data").get("room_count_cb")
            )
        )
        sreality_item["available_from"] = self.get_first_or_none(
            data.get("items"), "Datum nastěhování"
        )
        sreality_item["floor"] = self.get_first_or_none(data.get("items"), "Podlaží")
        sreality_item["area"] = self.get_first_or_none(
            data.get("items"), "Užitná plocha"
        )
        sreality_item["furnished"] = data.get("recommendations_data").get("furnished")
        sreality_item["status"] = self.get_first_or_none(
            data.get("items"), "Stav objektu"
        )
        sreality_item["ownership"] = data.get("codeItems").get("ownership")
        sreality_item["penb"] = self.get_first_or_none(
            data.get("items"), "Energetická náročnost budovy"
        )
        # design =
        ...
        sreality_item["gps_lat"] = data.get("recommendations_data").get(
            "locality_gps_lat"
        )
        sreality_item["gps_lon"] = data.get("recommendations_data").get(
            "locality_gps_lon"
        )
